//Tile Baker
//Creates g3z models out of the tile layers in a room.
...
key=ds_map_find_first(unique_bgs) repeat (ds_map_size(unique_bgs)) {
    ...
    with (tileholder) if (myname==key) {
        ...
        d3d_model_vertex_texture_color(model,x,y,depth,l/bgw,t/bgh,image_blend,image_alpha)
        d3d_model_vertex_texture_color(model,x+w*tilesx,y,depth,(l+w)/bgw,t/bgh,image_blend,image_alpha)
        d3d_model_vertex_texture_color(model,x,y+h*tilesy,depth,l/bgw,(t+h)/bgh,image_blend,image_alpha)
        d3d_model_vertex_texture_color(model,x,y+h*tilesy,depth,l/bgw,(t+h)/bgh,image_blend,image_alpha)
        d3d_model_vertex_texture_color(model,x+w*tilesx,y,depth,(l+w)/bgw,t/bgh,image_blend,image_alpha)
        d3d_model_vertex_texture_color(model,x+w*tilesx,y+h*tilesy,depth,(l+w)/bgw,(t+h)/bgh,image_blend,image_alpha)
        vc+=6
        if (vc>=31000) {
